Shader "MidairImaging/LorentzBlur"
{
    Properties
    {
        ...
        Pass
        {
            ...
            half4 frag(v2f i) : SV_Target
            {
                ...
                for (float x = -blur; x <= blur; x += 1)
                {
                    ...
                    // float weight = exp(-0.5 * pow(distance_normalized, 2) * 6.0);
                    // float weight = 1.183 * exp(-abs(distance_normalized - 0.003) / 0.063);
                    // float weight = 1.183 * exp(-abs(distance_normalized - 0.003) / laplaceCoefC);
                    float weight = 1.028 / (1 + pow(((distance_normalized - 0.004) / lorentzCoefC), 2));
                    weight_total += weight;
